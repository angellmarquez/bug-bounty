...
test.describe('Cifrado de la plataforma (administrador)', () => {
    test('la pantalla solo muestra el estado: no hay botón para generar claves', async ({
        page,
    }) => {
        ...
        await expect(
            page.getByText('La clave privada nunca se muestra'),
        ).toBeVisible();
        await expect(
            page.getByText('no requiere ninguna acción'),
        ).toBeVisible();
        ...
        await expect(
            page.getByRole('button', { name: /Generar/i }),
        ).toHaveCount(0);

        expect(errores, errores.join('\n')).toEqual([]);
    });

    test('los demás roles no entran a la pantalla de cifrado', async ({
        page,
    }) => {

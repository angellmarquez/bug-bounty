...
test.describe('Páginas de error', () => {
    test('un 403 se muestra con el diseño de la aplicación, sin pantalla blanca', async ({ page }) => {
        ...
        await expect(page.locator('[data-test="error-titulo"]')).toHaveText('No tienes permiso para ver esto');
    ...
    test('una dirección que no existe da un 404 con salida al panel', async ({ page }) => {
        await iniciarSesion(page, 'moderador');
        const respuesta = await page.goto('/esta-pagina-no-existe');
        expect(respuesta?.status()).toBe(404);

        await expect(page.locator('[data-test="error-titulo"]')).toHaveText('Página no encontrada');
        await expect(page.locator('[data-slot="sidebar"]').first()).toBeVisible();
        await expect(page.locator('[data-test="error-inicio"]')).toHaveText(/Ir al panel/);
    });

    test('un informe inexistente da 404 sin detalles internos', async ({ page }) => {
        await iniciarSesion(page, 'investigador');
        const respuesta = await page.goto('/reportes/999999');
        expect(respuesta?.status()).toBe(404);

        await expect(page.locator('[data-test="error-titulo"]')).toHaveText('Página no encontrada');
        await expect(page.locator('body')).not.toContainText('No query results');
    });

    test('un visitante sin sesión ve el 404 a pantalla completa con salida al inicio', async ({ page }) => {
        const respuesta = await page.goto('/esta-pagina-no-existe');
        expect(respuesta?.status()).toBe(404);
